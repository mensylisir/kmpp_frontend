  ...
  <layout-content header="工具">
    ...
    <el-row>
      <div v-loading="loading" v-for="tool in tools" :key="tool.name">
        <el-col :span="6">
          <el-card
            style="margin-left: 10px; margin-top: 10px; height: 180px"
            class="box-card"
          >
            <el-row>
              <el-col :span="8">
                <img
                  ...
                  :src="require('@/assets/images/tools/' + tool.logo)"
                />
              </el-col>
              <el-col :span="16">
            ...
            <div v-if="tool.status === 'Running'">
              <span v-if="!tool.frame" style="float: right; margin: 12px">{{
                $t("commons.status.running")
              }}</span>
              <el-button
                v-if="tool.frame"
                size="small"
                @click="openFrame(tool)"
                style="float: right; margin: 5px"
              >
            ...
            <el-button
              v-if="tool.higher_version && tool.status === 'Running'"
              @click="onUpgrade(tool)"
              size="small"
              style="float: right; margin: 5px"
    ...
    <el-dialog
      :title="$t('cluster.detail.tool.enable_title')"
      width="30%"
    ...
    >
      ...
      >
        <el-form-item
          :label="$t('cluster.detail.tag.namespace')"
          prop="vars.namespace"
          :rules="requiredRules"
        >
          <el-select
            style="width: 80%"
            filterable
            v-model="toolForm.vars['namespace']"
            clearable
          >
            <el-option v-for="item of namespaces" :key="item" :value="item">{{
              item
            }}</el-option>
          </el-select>
        </el-form-item>
        ...
        <div v-if="toolForm.name === 'chartmuseum'">
          <el-form-item :label="$t('cluster.detail.tool.enable_storage')">
            <el-switch
              style="width: 80%"
              v-model="toolForm.vars['persistence_enabled']"
            ></el-switch>
          </el-form-item>
          <div v-if="toolForm.vars['persistence_enabled']">
            <el-form-item
              :label="$t('cluster.detail.tool.storage_class')"
              prop="vars.persistence_storageClass"
              :rules="requiredRules"
            >
              <el-select
                style="width: 80%"
                filterable
                v-model="toolForm.vars['persistence_storageClass']"
                clearable
              >
                <el-option v-for="item of storages" :key="item" :value="item">{{
                  item
                }}</el-option>
              </el-select>
            </el-form-item>
            <el-form-item
              :label="$t('cluster.detail.tool.storage_size')"
              prop="vars.persistence_size"
              :rules="numberRules"
            >
              <el-input-number
                :step="1"
                step-strictly
                style="width: 80%"
                v-model="toolForm.vars['persistence_size']"
                clearable
              ></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tag.node')">
              <el-select
                style="width: 80%"
                filterable
                v-model="toolForm.vars['nodeSelector_kubernetes\\_io/hostname']"
                clearable
              >
                <el-option v-for="item of nodes" :key="item" :value="item">{{
                  item
                }}</el-option>
              </el-select>
            </el-form-item>
        ...
        <div v-if="toolForm.name === 'prometheus'">
          <el-form-item
            :label="$t('cluster.detail.tool.data_retention')"
            prop="vars.server_retention"
            :rules="numberRules"
          >
            <el-input-number
              :step="1"
              step-strictly
              style="width: 80%"
              v-model="toolForm.vars['server_retention']"
              clearable
            ></el-input-number>
          </el-form-item>
          <el-form-item :label="$t('cluster.detail.tool.enable_storage')">
            <el-switch
              style="width: 80%"
              v-model="toolForm.vars['server_persistentVolume_enabled']"
            ></el-switch>
          </el-form-item>
          <div v-if="toolForm.vars['server_persistentVolume_enabled']">
            <el-form-item
              :label="$t('cluster.detail.tool.storage_class')"
              prop="vars.server_persistentVolume_storageClass"
              :rules="requiredRules"
            >
              <el-select
                style="width: 80%"
                filterable
                v-model="toolForm.vars['server_persistentVolume_storageClass']"
                clearable
              >
                <el-option v-for="item of storages" :key="item" :value="item">{{
                  item
            ...
            <el-form-item
              :label="$t('cluster.detail.tool.storage_size')"
              prop="vars.server_persistentVolume_size"
              :rules="numberRules"
            >
              <el-input-number
                :step="1"
                step-strictly
                style="width: 80%"
                v-model="toolForm.vars['server_persistentVolume_size']"
                clearable
              ></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tag.node')">
              <el-select
                ...
                filterable
                v-model="
                  toolForm.vars['server_nodeSelector_kubernetes\\_io/hostname']
                "
                clearable
              >
                <el-option v-for="item of nodes" :key="item" :value="item">{{
        ...
        <div v-if="toolForm.name === 'logging'">
          <el-form-item
            :label="$t('cluster.detail.tool.search_index')"
            prop="vars.fluentd__elasticsearch_elasticsearch_logstashPrefix"
            :rules="requiredRules"
          >
            <el-input
              style="width: 80%"
              v-model="
                toolForm.vars[
                  'fluentd__elasticsearch_elasticsearch_logstashPrefix'
                ]
              "
              clearable
            ></el-input>
            <div>
              <span class="input-help">{{
                $t("cluster.detail.tool.search_index_help")
              }}</span>
            </div>
          </el-form-item>
          <el-form-item
            :label="$t('cluster.detail.tool.replicas')"
            prop="vars.elasticsearch_replicas"
            :rules="numberRules"
          >
            <el-input-number
              :step="1"
              ...
              style="width: 80%"
              @blur="checkReplicas"
              v-model="toolForm.vars['elasticsearch_replicas']"
              clearable
            ></el-input-number>
            <div v-if="isReplicasValid">
              <span class="input-help"
                >{{ $t("cluster.detail.tool.max_replicas_num") }} :
                {{ nodeNum }}</span
              >
            </div>
            <div v-if="!isReplicasValid">
          ...
          <el-form-item
            :label="$t('cluster.detail.tool.hip_memory')"
            prop="vars.elasticsearch_esJavaOpts_item"
            :rules="numberRules"
          >
            <el-input-number
              :step="1"
              step-strictly
              style="width: 80%"
              v-model="toolForm.vars['elasticsearch_esJavaOpts_item']"
              clearable
            ></el-input-number>
            <div>
              <span class="input-help">{{
                $t("cluster.detail.tool.default_hip_memory")
              }}</span>
            </div>
          </el-form-item>
          <el-form-item :label="$t('cluster.detail.tool.enable_storage')">
            <el-switch
              style="width: 80%"
              v-model="toolForm.vars['elasticsearch_persistence_enabled']"
            ></el-switch>
          </el-form-item>
          <div v-if="toolForm.vars['elasticsearch_persistence_enabled']">
            <el-form-item
              :label="$t('cluster.detail.tool.storage_class')"
              prop="vars.elasticsearch_volumeClaimTemplate_storageClassName"
              :rules="requiredRules"
            >
              <el-select
                style="width: 80%"
            ...
            <el-form-item
              :label="$t('cluster.detail.tool.storage_size')"
              prop="vars.elasticsearch_volumeClaimTemplate_resources_requests_storage"
              :rules="numberRules"
            >
              <el-input-number
                :step="1"
                ...
                v-model="
                  toolForm.vars[
                    'elasticsearch_volumeClaimTemplate_resources_requests_storage'
                  ]
                "
                clearable
              ></el-input-number>
        ...
        <div v-if="toolForm.name === 'loki'">
          <el-form-item :label="$t('cluster.detail.tool.enable_storage')">
            <el-switch
              style="width: 80%"
              v-model="toolForm.vars['loki_persistence_enabled']"
            ></el-switch>
          </el-form-item>
          <div v-if="toolForm.vars['loki_persistence_enabled']">
            <el-form-item
              :label="$t('cluster.detail.tool.storage_class')"
              prop="vars.loki_persistence_storageClassName"
              :rules="requiredRules"
            >
              <el-select
                style="width: 80%"
                filterable
                v-model="toolForm.vars['loki_persistence_storageClassName']"
                clearable
              >
                <el-option v-for="item of storages" :key="item" :value="item">{{
                  item
            ...
            <el-form-item
              :label="$t('cluster.detail.tool.storage_size')"
              prop="vars.loki_persistence_size"
              :rules="numberRules"
            >
              <el-input-number
                :step="1"
                step-strictly
                style="width: 80%"
                v-model="toolForm.vars['loki_persistence_size']"
                clearable
              ></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tag.node')">
              <el-select
                ...
                filterable
                v-model="
                  toolForm.vars['loki_nodeSelector_kubernetes\\_io/hostname']
                "
                clearable
              >
                <el-option v-for="item of nodes" :key="item" :value="item">{{
        ...
        <div v-if="toolForm.name === 'grafana'">
          <el-form-item
            :label="$t('cluster.detail.tool.password')"
            prop="vars.adminPassword"
            :rules="passwordRules"
          >
            <el-input
              @blur="checkPassword"
              type="password"
              style="width: 80%"
              v-model="toolForm.vars['adminPassword']"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item
            :label="$t('cluster.detail.tool.password_re')"
            prop="vars.adminPasswordRe"
            :rules="passwordRules"
          >
            <el-input
              @blur="checkPassword"
              type="password"
              style="width: 80%"
              v-model="toolForm.vars['adminPasswordRe']"
              clearable
            ></el-input>
            <div v-if="!isPasswordValid">
              <span class="input-error">{{
                $t("cluster.detail.tool.grafana_password_help")
              }}</span>
            </div>
          </el-form-item>
          <el-form-item :label="$t('cluster.detail.tool.enable_storage')">
          ...
          <div v-if="toolForm.vars['persistence_enabled']">
            <el-form-item
              :label="$t('cluster.detail.tool.storage_class')"
              prop="vars.persistence_storageClassName"
              :rules="requiredRules"
            >
              <el-select
                style="width: 80%"
                filterable
                v-model="toolForm.vars['persistence_storageClassName']"
                clearable
              >
                <el-option v-for="item of storages" :key="item" :value="item">{{
                  item
        ...
        <div v-if="toolForm.name === 'kubeapps'">
          <el-form-item :label="$t('cluster.detail.tool.enable_storage')">
            <el-switch
              style="width: 80%"
              v-model="toolForm.vars['postgresql_persistence_enabled']"
            ></el-switch>
          </el-form-item>
          <div v-if="toolForm.vars['postgresql_persistence_enabled']">
            <el-form-item
              :label="$t('cluster.detail.tool.storage_class')"
              prop="vars.global_storageClass"
              :rules="requiredRules"
            >
              <el-select
                style="width: 80%"
                filterable
                v-model="toolForm.vars['global_storageClass']"
                clearable
              >
                <el-option v-for="item of storages" :key="item" :value="item">{{
                  item
            ...
            <el-form-item
              :label="$t('cluster.detail.tool.storage_size')"
              prop="vars.postgresql_persistence_size"
              :rules="numberRules"
            >
              <el-input-number
                :step="1"
                step-strictly
                style="width: 80%"
                v-model="toolForm.vars['postgresql_persistence_size']"
                clearable
              ></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tag.node')">
              <el-select
                style="width: 80%"
                filterable
                v-model="toolForm.vars['nodeSelector']"
                clearable
              >
                <el-option v-for="item of nodes" :key="item" :value="item">{{
                  item
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogEnableVisible = false">{{
          $t("commons.button.cancel")
        }}</el-button>
        <el-button type="primary" :disabled="submitLoading" @click="enable()">{{
          $t("commons.button.ok")
        }}</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      :title="$t('cluster.detail.tool.err_title')"
      width="30%"
      :visible.sync="dialogErrorVisible"
    >
      <div style="margin: 0 50px">
        <span style="line-height: 30px">{{ conditions | errorFormat }}</span>
      </div>
      <div slot="footer" class="dialog-footer">
    ...
    <el-dialog
      :title="$t('cluster.detail.tool.info_title')"
      width="30%"
      :visible.sync="dialogDisableVisible"
    >
      <span>{{ $t("cluster.detail.tool.disable_show_msg") }}</span>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogDisableVisible = false">{{
          $t("commons.button.cancel")
        }}</el-button>
        <el-button type="primary" @click="disable(toolForm)">{{
          $t("commons.button.ok")
        }}</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      :title="$t('cluster.detail.tool.upgrade_title')"
      width="30%"
      :visible.sync="dialogUpgradeVisible"
    >
      <span
        >{{ toolForm.name }}: {{ toolForm.version }} --->
        {{ toolForm.higher_version }}</span
      >
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogUpgradeVisible = false">{{
          $t("commons.button.cancel")
        }}</el-button>
        <el-button type="primary" @click="upgrade(toolForm)">{{
          $t("commons.button.ok")
        }}</el-button>
      </div>
    </el-dialog>
  ...
  methods: {
    ...
    openFrame(item) {
      getSecret(this.clusterName).then((data) => {
        ...
        window.open(
          "" + item.url.replace("{cluster_name}", this.clusterName),
          "_blank"
        );
      });
    },
          ...
          for (const tool of this.tools) {
            if (tool.name === "loki") {
              this.conditions =
                tool.status === "Waiting"
                  ? ""
                  : this.$t("cluster.detail.tool.log_err_msg");
              break;
            }
          }
          break;
          ...
          for (const tool of this.tools) {
            if (tool.name === "prometheus") {
              this.conditions =
                tool.status === "Running"
                  ? ""
                  : this.$t("cluster.detail.tool.grafana_err_msg");
              break;
            }
          }
          break;
    ...
    enable() {
      this.$refs["toolForm"].validate((valid) => {
        if (valid && this.isPasswordValid && this.isReplicasValid) {
          this.loadMenu =
            this.toolForm.name === "logging" ||
            this.toolForm.name === "loki" ||
            this.toolForm.name === "prometheus";
    ...
    checkPassword() {
      if (
        this.toolForm.vars["adminPassword"] &&
        this.toolForm.vars["adminPasswordRe"]
      ) {
    ...
    checkReplicas() {
      this.isReplicasValid =
        this.toolForm.vars["elasticsearch_replicas"] <= this.nodeNum;
    },
    onErrorShow(item) {
