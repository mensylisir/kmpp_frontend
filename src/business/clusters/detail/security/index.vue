<template>
  <div style="padding-top: 24px">
    ...
    <el-dialog
      :title="$t('cluster.detail.security.cis_result')"
      width="70%"
      ...
      <div class="dialog">
        <el-scrollbar style="height: 100%">
          <div align="center">
            ...
            >
              <el-table-column
                :label="$t('cluster.detail.security.code')"
                min-width="10%"
                prop="number"
              />
              <el-table-column
              ...
              />
              <el-table-column
                :label="$t('cluster.detail.security.description')"
                min-width="40%"
                prop="desc"
              />
              <el-table-column
                :label="$t('cluster.detail.security.advise')"
                min-width="40%"
                prop="remediation"
              />
            </complex-table>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      listCisByPage(this.clusterName, currentPage, pageSize)
        .then((data) => {
          this.loading = false;
    ...
    cisCreate() {
      this.$confirm(
        this.$t("cluster.detail.security.start_cis"),
        this.$t("commons.button.confirm"),
        {
    ...
    polling() {
      this.timer = setInterval(() => {
        ...
        if (flag) {
          const { currentPage, pageSize } = this.paginationConfig;
          listCisByPage(this.clusterName, currentPage, pageSize).then(
            (data) => {
              this.data = data.items;
              this.paginationConfig.total = data.total;
            }
