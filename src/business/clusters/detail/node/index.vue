<template>
  <div>
    <el-alert
      v-if="provider === ''"
      :title="$t('cluster.detail.node.operator_help')"
      type="info"
    />
    <complex-table
    ...
    >
      ...
      >
        <template v-slot:default="{ row }">
          <el-link
            v-if="row.status.indexOf('Running') !== -1"
            type="info"
            @click="getDetailInfo(row)"
            >{{ row.name }}</el-link
          >
          <span v-if="row.status.indexOf('Running') === -1">{{
            row.name
          }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template v-slot:default="{ row }">
          <div
            v-if="
              row.status.indexOf('Terminating') !== -1 &&
              currentCluster.provider !== 'bareMetal'
            "
          >
          ...
          >
            <i class="el-icon-loading" /> &nbsp; &nbsp; &nbsp;
            <el-link type="info" @click="getStatus(row)"
              >{{ $t("commons.status.terminating") }}
            </el-link>
          </div>
    ...
    >
      ...
      >
        <el-form-item
          v-if="provider === 'plan'"
          prop="increase"
          :label="$t('cluster.detail.node.increment')"
        >
          <el-input-number
            style="width: 80%"
            v-model.number="createForm.increase"
            clearable
          />
        </el-form-item>

        <el-form-item
          v-if="provider === 'bareMetal'"
          prop="hosts"
          :label="$t('cluster.detail.node.host')"
        >
          <el-select
            style="width: 80%"
        ...
        <el-form-item
          v-if="supportGpu === 'disable' && !gpuExist"
          prop="supportGpu"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          :disabled="provider === '' || submitLoading"
          @click="submitCreate()"
    ...
    <!-- 节点详情 -->
    <el-drawer
      :title="$t('cluster.detail.node.node_detail')"
          ...
          <div>
              ...
              <table style="width: 100%" class="myTable">
                <tbody>
                  ...
                  <tr>
                    <td>Container Runtime Version</td>
                    <td>
                      {{ detaiInfo.status.nodeInfo.containerRuntimeVersion }}
                    </td>
                  </tr>
                  <tr>
                    <td>Kubelet Version</td>
            ...
            <div style="margin-top: 16px; margin-bottom: 24px">
              <table style="width: 100%" class="myTable">
                <tbody>
                  <tr
                    v-for="(value, name) in detaiInfo.metadata.labels"
                    :key="name"
                  >
                    <td>{{ name }}</td>
                    <td>{{ value }}</td>
            ...
            <div style="margin-top: 16px; margin-bottom: 24px">
              <el-table
                :data="detaiInfo.status.conditions"
                border
                style="width: 100%"
              >
                <el-table-column
        ...
        <el-scrollbar style="height: 100%">
          <div>
            ...
            >
              <el-step
                :title="log.name"
                :description="log.message | errorFormat"
              >
    ...
    <el-dialog
      :title="$t('cluster.detail.node.cordon')"
      width="50%"
      :visible.sync="dialogCordonVisible"
    >
      <el-row type="flex" justify="center">
        <el-form label-width="120px">
          <el-form-item :label="$t('cluster.detail.node.mode')">
            <el-radio v-model="modeSelect" label="safe">{{
              $t("cluster.detail.node.safe")
            }}</el-radio>
            <div>
              <span class="input-help">{{
                $t("cluster.detail.node.safe_cordon_help")
              }}</span>
            </div>
            <el-radio v-model="modeSelect" label="force">{{
              $t("cluster.detail.node.force")
            }}</el-radio>
            <div>
              <span class="input-help">{{
                $t("cluster.detail.node.force_drain_help1")
              }}</span>
              <div>
                <span class="input-help" style="margin-left: 20px">{{
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogCordonVisible = false">{{
          $t("commons.button.cancel")
        }}</el-button>
        <el-button
          type="primary"
          :disabled="submitLoading"
          @click="submitCordon(true)"
          >{{ $t("commons.button.submit") }}</el-button
        >
      </div>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    buttonDisabled(row) {
      const onPolling = [
        "Initializing",
        "Terminating",
        "Terminating, SchedulingDisabled",
        "Creating",
      ];
      if (row) {
    ...
    onDelete(row) {
      if (row) {
        if (row.roles.indexOf("master") !== -1) {
          this.$message({
            type: "info",
            message: this.$t("cluster.detail.node.is_master_node"),
          });
          return;
        }
      } else {
      ...
      if (operation === "cordon") {
        for (const item of this.selects) {
          if (item.status !== "Running") {
            this.$message({
              type: "info",
              message: this.$t("cluster.detail.node.existing_cordoned"),
            });
            return;
          }
        }
        ...
        for (const item of this.selects) {
          if (item.status !== "Running, SchedulingDisabled") {
            this.$message({
              type: "info",
              message: this.$t("cluster.detail.node.existing_actived"),
            });
            return;
          }
        }
        this.$confirm(
          this.$t("commons.confirm_message.uncordon"),
          this.$t("commons.message_box.prompt"),
          {
            confirmButtonText: this.$t("commons.button.confirm"),
            cancelButtonText: this.$t("commons.button.cancel"),
    ...
    bacthDeletePod(datas, nodeName) {
      ...
      for (const pod of datas) {
        if (
          ...
          pod.metadata.ownerReferences.kind !== "daemonset"
        ) {
          const rmPod = {
    ...
    getStatus(row) {
      ...
      getNodeByName(this.clusterName, this.currentNode.name).then((data) => {
        this.log.icon =
          data.status === "Failed" ? "el-icon-close" : "el-icon-loading";
        const status = data.status === "Failed" ? data.pre_status : data.status;
        this.log.name =
          status === "Initializing"
            ? this.$t("cluster.detail.node.node_expand")
            : this.$t("cluster.detail.node.node_shrink");
    ...
    dialogPolling() {
      this.timer2 = setInterval(() => {
        if (
          this.log.phase !== "Running" &&
          this.log.phase !== "Failed" &&
          this.log.phase !== "Failed"
        ) {
