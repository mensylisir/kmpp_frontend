<template>
  <div>
    <el-alert
      v-if="provider === ''"
      :title="$t('cluster.detail.storage.operator_help')"
      type="info"
    />
          ...
          <el-card>
            <template>
              <el-button-group v-permission="['ADMIN']">
                <el-button size="small" @click="pvCreate()">{{
                  $t("commons.button.create")
                }}</el-button>
                <el-button
                  size="small"
                  :disabled="pvSelection.length < 1"
                  @click="onBatchDelete('pv')"
            ...
            >
              ...
              />
              <el-table-column
                :label="$t('cluster.detail.storage.capacity')"
                min-width="100"
              ...
              />
              <el-table-column
                :label="$t('cluster.detail.storage.source')"
                min-width="100"
            ...
            <k8s-page
              @pageChange="pvPageChange"
              :nextToken="pvPage.nextToken"
            />
          </el-card>
        </el-tab-pane>
        <el-tab-pane
          :label="$t('cluster.detail.storage.storage_class')"
          name="storage_class"
        >
          <el-card>
            <template>
              <el-button-group v-permission="['ADMIN']">
                <el-button size="small" @click="classCreate()">{{
                  $t("commons.button.create")
                }}</el-button>
                <el-button
                  size="small"
                  :disabled="classSelection.length < 1"
                  @click="onBatchDelete('class')"
                  >{{ $t("commons.button.delete") }}</el-button
                >
              </el-button-group>
            </template>
            <complex-table
              style="margin-top: 20px"
              v-loading="loading"
              :selects.sync="classSelection"
              :data="storageClassDatas"
            >
              <el-table-column type="selection" fix></el-table-column>
              <el-table-column
                :label="$t('commons.table.name')"
              ...
              />
              <el-table-column
                :label="$t('cluster.detail.storage.provisioner_short')"
                min-width="100"
                prop="provisioner"
                fix
              />
              <el-table-column
                :label="$t('cluster.detail.storage.reclaim_policy')"
                min-width="100"
                prop="reclaimPolicy"
                fix
              />
              <el-table-column
                :label="$t('cluster.detail.storage.volume_binding_mode')"
                min-width="100"
            ...
            <k8s-page
              ...
              :nextToken="classPage.nextToken"
            />
          </el-card>
        </el-tab-pane>
        <el-tab-pane
          :label="$t('cluster.detail.storage.provisioner')"
          name="provisioner"
        >
          <el-card>
            <template>
              <el-button-group v-permission="['ADMIN']">
                <el-button
                  size="small"
                  :disabled="provider === ''"
                  @click="provisionerCreate()"
                  >{{ $t("commons.button.create") }}</el-button
                >
                <el-button
                  size="small"
                  :disabled="provisionerSelection.length < 1"
                  @click="onSync()"
                ...
                <el-button
                  size="small"
                  :disabled="provisionerSelection.length < 1"
                  @click="onBatchDelete('provisioner')"
                  >{{ $t("commons.button.delete") }}</el-button
                >
              </el-button-group>
            </template>
            <complex-table
              style="margin-top: 20px"
              v-loading="loading"
              :selects.sync="provisionerSelection"
              :data="provisionerDatas"
            >
              <el-table-column type="selection" fix></el-table-column>
              <el-table-column
                :label="$t('commons.table.name')"
    ...
    >
      ...
      <ul
        style="margin-left: 5%"
        :key="pro.name"
        v-for="pro of provisionerSelection"
      >
    ...
    >
      <div style="margin: 0 50px">
        <span style="line-height: 30px">{{ errMsg | errorFormat }}</span>
      </div>
      <div slot="footer" class="dialog-footer">
      ...
      if (this.activeName === "pv") {
        listPersistentVolumes(this.clusterName, this.pvPage.continueToken)
          .then((data) => {
            ...
            this.pvPage.nextToken = data.metadata["continue"]
        ...
        listStorageClass(this.clusterName, this.classPage.continueToken, false)
          .then((data) => {
            ...
            this.classPage.nextToken = data.metadata["continue"]
              ? data.metadata["continue"]
              : "";
          })
          .catch(() => {
        ...
        case "provisioner":
          listStorageClass(this.clusterName).then((data) => {
            ...
            if (isClassExist) {
              this.$message({
                type: "error",
                message: this.$t("cluster.detail.storage.storage_class_exist"),
              });
            } else {
    ...
    submitSync() {
      syncProvisioner(this.clusterName, this.provisionerSelection)
        .then(() => {
          ...
          this.$message({
            type: "success",
            message: this.$t("cluster.detail.storage.start_provisioner_sync"),
          });
