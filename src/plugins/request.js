...
instance.interceptors.request.use(
  config => {
    ...
    config.headers.Authorization =
      "Bearer " +
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..fMIluKojKISMVIxryDjPkqjkmdFA-KV2wKorjcUk0O0";
    return config;
  },
  error => {
...
const checkAuth = response => {
  // 请根据实际需求修改
  if (
    response.headers["authentication-status"] === "invalid" ||
    response.status === 401
  ) {
...
export const del = (url, data, loading) => {
  return promise(request({ url: url, method: "delete", data }), loading);
};

export const patch = (url, data, headers, loading) => {
  if (headers) {
    return promise(
      request({ url: url, headers: headers, method: "patch", data }),
      loading
    );
  }
