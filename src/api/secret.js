...
export function getSecret(clustername, namespace) {
  const url = namespace
    ? `/api/v1/kubernetes/secret?clustername=${clustername}&namespace=${namespace}`
    : `/api/v1/kubernetes/secret?clustername=${clustername}`;
  return get(url);
}
...
export function createConfig(data) {
  return post(`/api/v1/kubernetes/secret`, data);
}

// 删除Deploy
export function delJob(data) {
  return del(`/api/v1/kubernetes/secret`, data);
}
...
export function getConfigItem(clustername, namespace, name) {
  return get(
    `/api/v1/kubernetes/secret?clusterName=${clustername}&namespace=${namespace}&name=${name}`
  );
}
...
export function updateJob(data) {
  return post("/api/v1/kubernetes/secret/update", data);
}
